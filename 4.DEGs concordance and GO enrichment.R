...
# 3.Subset spot from the ST-seq datasets
...
DEGs_up_list_GO <- compareCluster(DEGs_up_list, fun="enrichGO",OrgDb ="org.Mm.eg.db", 
                                  pvalueCutoff=0.01,keyType ="SYMBOL")
dotplot(DEGs_up_list_GO,showCategory = 1) + ggtitle("Up regulated DEGs") + xlab(NULL) +
  theme(axis.text.x = element_text(angle=45, hjust=1, vjust=1)) + scale_color_gradientn(colours = rev(viridis(20)), guide = guide_colorbar(ticks.colour = "black",frame.colour = "black"), name = "p.adjust") 

# cnetplot(DEGs_up_list_GO,showCategory = 1,pie = "count",
#          pie_scale = 1,cex_label_category = 1.2,cex_label_gene = 0,
#          color_category = "black",node_label="category",layout="nicely",colorEdge=T,categorySize="count") #+ 
#   scale_fill_manual(values = c("AAN_2W_vs_Control_up" = '#58A4C3',"AAN_4W_vs_Control_up" = '#E95C59'), name = "")
